...
impl FuseSession {
    ...
    pub fn new_with_autounmount(
        ...
        Ok(FuseSession {
            mountpoint: dest,
            fuse_device: FUSE_DEVICE.to_owned(),
    ...
    /// Override location of fuse device
    pub fn set_fuse_device(&mut self, device_path: String) {
        self.fuse_device = device_path;
    }
...
fn fuse_kern_mount(
    ...
    if auto_unmount {
        ...
        match mount(
            ...
            Ok(()) => Ok((file, None)),
            Err(nix::errno::Errno::EPERM) if allow_fusermount => {
                fuse_fusermount_mount(mountpoint, fsname, subtype, opts, flags, auto_unmount)
            }
...
fn fuse_fusermount_mount(
    ...
    if auto_unmount {
        ...
        let output = proc.wait_with_output().map_err(IoError)?;
...
mod tests {
    use super::*;
    use std::{fs::File, os::unix::io::FromRawFd, path::Path};
...
mod asyncio {
    ...
    use crate::{
        ...
        transport::{FuseBuf, Reader, Writer},
    };
    ...
    mod tests {
        ...
        use crate::{
            api::{Vfs, VfsOptions},
            async_util::{AsyncDriver, AsyncExecutor},
        };
