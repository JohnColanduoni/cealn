...
impl<'a> Authorization {
    ...
    pub fn copy_rights_sync(&self, rights: AuthorizationItemSetStorage, environment: Option<AuthorizationItemSetStorage>, flags: Flags) -> Result<AuthorizationItemSet<'_>> {
        let rights_ptr = &rights.set as *const sys::AuthorizationItemSet;

        let env_ptr = environment.as_ref().map_or(std::ptr::null(), |e| {
            &e.set as *const sys::AuthorizationItemSet
        });

        let mut output = MaybeUninit::<*mut sys::AuthorizationItemSet>::uninit();

        let status = unsafe { sys::AuthorizationCopyRights(self.handle, rights_ptr, env_ptr, flags.bits(), output.as_mut_ptr()) };
        ...
        let set = AuthorizationItemSet {
            inner: unsafe { output.assume_init() },
            phantom: PhantomData,
        };
