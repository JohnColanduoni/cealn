use regex::Regex;
use std::{collections::HashMap, env, ffi::OsString, fmt, iter::FromIterator, process::Command};
...
#[cfg(target_os = "macos")]
fn get_rustc_link_lib(expected_version: &PythonVersion, ld_version: &str, _: bool) -> Result<String, String> {
...
fn find_interpreter_and_get_config(
    ...
    let mut possible_names = vec!["python".to_string(), format!("python{}", expected_version.major)];
    ...
    Err(format!("No python interpreter found of version {}", expected_version))
}
...
fn configure_from_path(expected_version: &PythonVersion) -> Result<String, String> {
    let (interpreter_version, interpreter_path, lines) = find_interpreter_and_get_config(expected_version)?;
    ...
    let link_mode_unresolved_static = watched_var_os("CARGO_FEATURE_LINK_MODE_UNRESOLVED_STATIC").is_some();
...
fn main() {
    ...
    // CEALN PATCH: use static values for wasi here
    // if !env::var("TARGET").unwrap().ends_with("-wasi") {
    //    panic!("patched for WASI only");
    //}
    ...
    let mut config_map: HashMap<String, String> = HashMap::from_iter(
        [
            ("Py_USING_UNICODE".to_owned(), "1".to_owned()),
            ("Py_UNICODE_WIDE".to_owned(), "1".to_owned()),
            ("WITH_THREAD".to_owned(), "0".to_owned()),
            (
                "Py_DEBUG".to_owned(),
            ...
            ("Py_REF_DEBUG".to_owned(), "0".to_owned()),
            // FIXME: not sure this is right
            ("Py_TRACE_REFS".to_owned(), "0".to_owned()),
            // FIXME: not sure this is right
            ("COUNT_ALLOCS".to_owned(), "0".to_owned()),
            ("Py_UNICODE_SIZE".to_owned(), "4".to_owned()),
        ]
        .iter()
        .cloned(),
    );
    if is_not_none_or_zero(config_map.get("Py_DEBUG"))
        && (version.major < 3 || (version.major == 3 && version.minor.map(|x| x < 8) == Some(true)))
    {
        config_map.insert("Py_TRACE_REFS".to_owned(), "1".to_owned()); // Py_DEBUG implies Py_TRACE_REFS for Python < 3.8
    }
