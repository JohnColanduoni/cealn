...
impl DockerConfig {
    ...
    pub fn get_credentials(&self, registry: &str) -> anyhow::Result<Option<Credential>> {
        let Some(cred_helper) = self.cred_helpers.get(registry).or_else(|| self.creds_store.as_ref()) else {
            return Ok(None);
        };
