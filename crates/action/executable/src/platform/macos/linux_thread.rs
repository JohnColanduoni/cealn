...
use crate::platform::{linux_process::_LinuxProcess, mman::VmRegion, process::_Process, sys::arm_thread_state64_t};

pub(super) struct LinuxThread {
    host_thread: JoinHandle<Result<()>>,
}

impl LinuxThread {
    pub fn new(process: Arc<_LinuxProcess>, thread_init_state: arm_thread_state64_t) -> Result<LinuxThread> {
        let host_thread = std::thread::Builder::new()
            .name("vm-thread".to_owned())
            .spawn(move || run(process, thread_init_state))?;
        Ok(LinuxThread { host_thread })
    }
...
fn run(process: Arc<_LinuxProcess>, thread_init_state: arm_thread_state64_t) -> Result<()> {
    unsafe {
        let mut cpu = 0;
